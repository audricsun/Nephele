...
class Class(Model):
    name = models.CharField(
        max_length=255,
        db_comment="Name of the storage class",
    )
    ...
    zone = models.ForeignKey(
        ...
        related_name="class_in_zone",
    )
    root_path = models.CharField(
        max_length=255,
        validators=[validate_storage_class_host_path_root],
    )

    capacity = models.IntegerField(
        default=0,
        db_column="max capacity of storage service",
    )

    capacity_inodes = models.IntegerField(
        default=0,
        db_column="max capacity of inodes",
    )
...
class Quota(Model):
    ...
    def __str__(self):
        return (
            f"Quota::{self.project.name}::{self.used()}/{self.limit}[{self.provider}]"
        )
